...
class RoutingUrlGeneratorTest extends TestCase
{
	...
	public function testBasicRouteGeneration()
	{
		 ...
		 * Empty Named Route
		 */
		$route = new Route(array('GET'), '/', array('as' => 'plain'));
		$routes->add($route);

		/**
		 * Named Routes
		 */
		$route = new Route(array('GET'), 'foo/bar', array('as' => 'foo'));
		$routes->add($route);

		/**
		 * Parameters...
		 */
		$route = new Route(array('GET'), 'foo/bar/{baz}/breeze/{boom}', array('as' => 'bar'));
		$routes->add($route);

		/**
		 * HTTPS...
		 */
		$route = new Route(array('GET'), 'foo/bar', array('as' => 'baz', 'https'));
		$routes->add($route);

		/**
		 * Controller Route Route
		 */
		$route = new Route(array('GET'), 'foo/bar', array('controller' => 'foo@bar'));
		$routes->add($route);

		/**
		 * Non ASCII routes
		 */
		$route = new Route(array('GET'), 'foo/bar/åαф/{baz}', array('as' => 'foobarbaz'));
		$routes->add($route);
	...
	public function testHttpOnlyRoutes()
	{
		 ...
		 * Named Routes
		 */
		$route = new Route(array('GET'), 'foo/bar', array('as' => 'foo', 'http'));
		$routes->add($route);
	...
	public function testRoutesWithDomains()
	{
		 ...
		 * Wildcards & Domains...
		 */
		$route = new Route(array('GET'), 'foo/bar/{baz}', array('as' => 'bar', 'domain' => 'sub.{foo}.com'));
		$routes->add($route);
	...
	public function testHttpsRoutesWithDomains()
	{
		 ...
		 */
		$route = new Route(array('GET'), 'foo/bar', array('as' => 'baz', 'domain' => 'sub.foo.com'));
		$routes->add($route);
